...
// (pubKey, currExpirationTime, currRegistered) is in the merkle tree.
// It then verifies that if that same leaf is updated with the hash of
// (pubKey, updatedExpirationTime, updatedRegistered), then its path to the
// root will be equal to updatedPath.
template UpdateVerifier(levels) {
    signal input pubKey;

    signal input currExpirationTime;
    ...
    signal input updatedExpirationTime;
    ...
    currLeafHasher.inputs[1] <== currExpirationTime;
    ...
    updatedLeafHasher.inputs[1] <== updatedExpirationTime;
...
component main {public [pubKey, currExpirationTime, currRegistered, updatedExpirationTime, updatedRegistered, currPath, updatedPath] } = UpdateVerifier(20);