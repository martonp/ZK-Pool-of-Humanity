...
export class MerkleTree {
  ...
  async path(index) {
    class PathTraverser {
      constructor(prefix, storage, zero_values, hasher) {
        this.prefix = prefix
        this.storage = storage
      ...
      async handle_index(level, element_index, sibling_index) {
        ...
        if (element_index % 2 == 0) {
          this.path.push(this.hasher.hash(element, sibling))
        } else {
          this.path.push(this.hasher.hash(sibling, element))
        }
      }
    }
    index = Number(index)
  ...
  async updatedPath(index, element) {
    try {
      ...
      traverser.key_values_to_put.push({
        key: MerkleTree.index_to_key(this.prefix, 0, index),
        value: element
      });
      ...
      return traverser.key_values_to_put.map((kv) => { return kv.value })
    } catch (e) {
      console.error(e)
    }
  }
